...
export default {
  ...
  async test({credentials,discovery}){
    ...
    const r=await timedFetch(endpoint,{method:'POST',headers:{'content-type':'application/json','accept':'application/json','apikey':cfg.anonKey,'authorization':`Bearer ${cfg.anonKey}`,'user-agent':'LIMPO-Auth-Engine/2.0'},body:JSON.stringify({email:String(credentials.username||''),password:String(credentials.password||'')})});
    let payload={};try{payload=await r.json();}catch{}
    if(r.ok&&(payload?.access_token||payload?.user))return{status:'LOGIN_SUCCESS',success:true,httpStatus:r.status,transport:'supabase-password',sessionCookieSet:false,reason:'O Supabase Auth aceitou a credencial. Tokens não são retornados pelo LIMPO.'};
    const msg=String(payload?.error_description||payload?.msg||payload?.error||'').toLowerCase();
    if(r.status===400||r.status===401||/invalid login|invalid credentials|email not confirmed|user not found|wrong password/.test(msg))return{status:'LOGIN_FAILED',success:false,httpStatus:r.status,transport:'supabase-password',sessionCookieSet:false,reason:'O Supabase Auth rejeitou a credencial.'};
