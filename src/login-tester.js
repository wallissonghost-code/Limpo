...
function chooseForm(html,base){const forms=[...html.matchAll(/<form\b([^>]*)>([\s\S]*?)<\/form\s*>/gi)].map((m,index)=>{const fa=attrs(`<form ${m[1]}>`);const inputs=[...m[2].matchAll(/<input\b[^>]*>/gi)].map(x=>attrs(x[0]));let user=null,pass=null;for(const a of inputs){const us=scoreUser(a),ps=scorePass(a);if(us>0&&(!user||us>user.score))user={a,score:us};if(ps>0&&(!pass||ps>pass.score))pass={a,score:ps};}const score=(user?.score||0)+(pass?.score||0)+(pass?10:0);return{index,fa,inputs,user,pass,score};}).sort((a,b)=>b.score-a.score);
 ...
 const baseUrl=new URL(base);const action=parseTarget(new URL(best.fa.action||base,base).toString());const crossOrigin=action.origin!==baseUrl.origin;
 if(action.protocol!=='https:')throw new Error('O destino do login não usa HTTPS. O LIMPO não envia senha por conexão insegura.');
 const method=(best.fa.method||'get').toLowerCase();if(method!=='post')throw new Error('O formulário não usa POST. O LIMPO não envia senha pela URL.');
 ...
 return{action:action.toString(),method,userName:best.user.a.name||null,passName:best.pass.a.name||null,hidden,crossOrigin,loginOrigin:baseUrl.origin,submitOrigin:action.origin};
}
...
export async function testLogin(rawUrl,email,password){if(!email||!password)throw new Error('Informe e-mail/usuário e senha.');if(String(password).length>512||String(email).length>320)throw new Error('Credenciais inválidas.');
 ...
 const submit=await timedFetch(form.action,{method:'POST',redirect:'manual',headers:{'user-agent':'LIMPO-Login-Test/1.1','accept':'text/html,application/xhtml+xml,application/json,*/*;q=0.5','content-type':'application/x-www-form-urlencoded','origin':new URL(finalUrl).origin,'referer':finalUrl.toString()},body:body.toString()});
 ...
 return{ok:true,status,success,httpStatus:submit.status,loginPage:finalUrl.toString(),submitUrl:form.action,submitOrigin:form.submitOrigin,crossOrigin:form.crossOrigin,redirectTo:location?new URL(location,form.action).toString():null,sessionCookieSet:cookieSet,reason,note:form.crossOrigin?'O formulário usa um domínio de autenticação diferente do site. O LIMPO permitiu o envio porque o destino usa HTTPS e passou pela validação de URL. Nenhuma senha, cookie ou token é retornado.':'Resultado heurístico de uma única tentativa. O LIMPO não retorna nem armazena senha, valor de cookie ou token.'};}
