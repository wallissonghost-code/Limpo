...
async function fetchTarget(start){
  ...
  for(let i=0;i<=MAX_REDIRECTS;i++){
    const r=await timedFetch(current.toString(),{redirect:'manual',headers:{'user-agent':'LIMPO-Auth-Analyzer/4.0','accept':'text/html,application/xhtml+xml,text/plain,application/javascript,application/json,*/*;q=0.5'}});
...
async function inspectMap(url,ctx){
  try{
    const target=parseTarget(url);const r=await timedFetch(target.toString(),{redirect:'follow',headers:{'user-agent':'LIMPO-Auth-Analyzer/4.0','accept':'application/json,text/plain,*/*;q=0.3'}},4500);
    ...
    ctx.corpus.push(text);inspectBehavior(ctx,text,`sourcemap:${target.pathname}`,target.toString());ctx.coverage.sourceMapsAnalyzed++;return x.bytes;
  }catch{return 0;}
}

async function inspectScript(url,ctx){
  let target;try{target=parseTarget(url);}catch{return{bytes:0,partial:true,chunks:[],maps:[]};}
  let r;try{r=await timedFetch(target.toString(),{redirect:'follow',headers:{'user-agent':'LIMPO-Auth-Analyzer/4.0','accept':'application/javascript,text/javascript,*/*;q=0.4','range':`bytes=0-${SCRIPT_SLICE-1}`}},5000);}catch{return{bytes:0,partial:true,chunks:[],maps:[]};}
