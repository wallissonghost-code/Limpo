...
async function fetchText(url) {
  const r = await fetch(url, { redirect: 'follow', headers: { 'User-Agent': 'LIMPO-Auth-Detector/3.1' } });
  ...
  if (type && !/(text|javascript|json|html|xml|css)/.test(type)) return { ok:r.ok, status:r.status, text:'', finalUrl, headers:{} };
  const text = r.ok ? (await r.text()).slice(0, MAX_TEXT) : '';
  const headers = {};
  for (const name of ['server','via','x-powered-by','www-authenticate','set-cookie','location','cf-ray','x-vercel-id','x-amz-cf-id','content-security-policy']) {
    const v = r.headers.get(name); if (v) headers[name] = v.slice(0, 2500);
  }
...
function extractResources(text, baseUrl) {
  ...
  const patterns = [
    /<script[^>]+src=["']([^"']+)["'][^>]*>/gi,
    /<link[^>]+(?:rel=["'](?:modulepreload|preload|prefetch)["'][^>]+href|href=["']([^"']+)["'][^>]+rel=["'](?:modulepreload|preload|prefetch)["'])[^>]*>/gi,
    /(?:import\s+(?:[^"']+?\s+from\s+)?|import\s*\()["']([^"']+)["']/g,
    /new\s+Worker\s*\(\s*["']([^"']+)["']/g
  ];
  for (const p of patterns) {
    let m;
    while ((m = p.exec(source)) && out.size < MAX_SCRIPTS * 2) {
      const raw = m[1] || m[2];
      const u = safeUrl(raw, baseUrl);
      if (!u) continue;
      if (/\.(?:js|mjs|cjs)(?:$|\?)/i.test(u.pathname + u.search) || /(?:script|chunk|bundle|auth|oauth|login|identity|sdk)/i.test(u.href)) out.add(u.href);
    }
  }
  return [...out];
}
...
const PROVIDERS = [
  ...
  ['supabase', [/\.supabase\.co|supabase-js|createClient\s*\(/i, /auth\.signInWithPassword|signInWithOtp|signInWithOAuth/i]],
  ['auth0', [/auth0\.com|@auth0|createAuth0Client|Auth0Client/i, /\/authorize|\/oauth\/token/i]],
  ['clerk', [/clerk\.com|@clerk|clerkpublishablekey|clerk-js|clerkClient/i, /__clerk|clerk\.accounts/i]],
...
function scoreText(text, source, scoreMap, evidenceMap) {
  ...
  for (const [provider, patterns] of PROVIDERS) {
    ...
    if (arr.length < 10) arr.push(`${source}: ${hits} sinal(is)`);
...
function scoreEndpoint(endpoint, scoreMap, evidenceMap) {
  ...
  for (const [p,re] of rules) if (re.test(e)) {
    ...
    const arr=evidenceMap.get(p)||[]; if(arr.length<10) arr.push(`endpoint: ${endpoint}`); evidenceMap.set(p,arr);
  }
  if (/\/authorize|\/token|\/userinfo|openid-configuration/.test(e)) {
    for (const p of ['oauth2','oidc']) { scoreMap.set(p,(scoreMap.get(p)||0)+1); const arr=evidenceMap.get(p)||[]; if(arr.length<10) arr.push(`endpoint padrão: ${endpoint}`); evidenceMap.set(p,arr); }
  }
}
...
async function probeOidc(baseUrl, endpoints) {
  ...
  for (const href of [...candidates].slice(0,6)) {
    try {
      ...
      const r = await fetch(u, { redirect:'follow', headers:{'User-Agent':'LIMPO-Auth-Detector/3.1'} });
      if(!r.ok) continue;
...
function mergeStorage(dst, src) {
  for (const key of ['localStorage','sessionStorage','cookies','jwtClaims']) {
    for (const value of src[key] || []) if (!dst[key].includes(value) && dst[key].length < 20) dst[key].push(value);
  }
}
...
export async function detectAuthProviders(target) {
  ...
  while (queue.length && visited.size < MAX_SCRIPTS) {
    const item = queue.shift();
    if (!item || visited.has(item.href)) continue;
    visited.add(item.href);
    try {
      const resourceUrl = safeUrl(item.href);
      ...
      const label = `${resourceUrl.origin === first.finalUrl.origin ? 'script' : 'external-script'}:${resourceUrl.hostname}${resourceUrl.pathname}`;
      scoreText(loaded.text, label.slice(0,180), scoreMap, evidenceMap);
      ...
      mergeStorage(storage, extractStorage(loaded.text));
      if (item.depth < MAX_DEPTH) {
        for (const child of extractResources(loaded.text, loaded.finalUrl)) {
          if (!visited.has(child) && queue.length < MAX_SCRIPTS * 2) queue.push({href:child, depth:item.depth+1});
        }
      }
    } catch {}
  }
  ...
  for (const meta of oidcMetadata) {
    ...
    const arr=evidenceMap.get('oidc')||[]; if(arr.length<10) arr.push(`metadado OIDC público: ${meta.url}`); evidenceMap.set('oidc',arr);
    const issuer=String(meta.issuer||'');
    for (const [p,re] of [['auth0',/auth0\.com/i],['okta',/okta\.com/i],['entra-id',/microsoftonline\.com/i],['cognito',/amazoncognito/i],['keycloak',/\/realms\//i]]) if(re.test(issuer)){scoreMap.set(p,(scoreMap.get(p)||0)+5);const a=evidenceMap.get(p)||[];if(a.length<10)a.push(`issuer OIDC: ${issuer}`);evidenceMap.set(p,a);}
  }
  ...
  if (storage.jwtClaims.length >= 2 || /access[_-]?token|refresh[_-]?token/i.test(first.text)) {
    ...
    const arr=evidenceMap.get('jwt-custom')||[]; if(arr.length<10) arr.push(`claims/chaves JWT observados: ${storage.jwtClaims.slice(0,8).join(', ')}`); evidenceMap.set('jwt-custom',arr);
  }
